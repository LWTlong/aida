...
import { mergeRuleRegistry } from '../../utils/rules.js';
import { mergeSkillRegistry } from '../../utils/skills.js';
...
export async function merge(): Promise<void> {
  ...
  const rules = mergeRuleRegistry(projectRoot);
  const skills = mergeSkillRegistry(projectRoot);
