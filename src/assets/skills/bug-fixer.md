...
4. **记录反馈（强制，不可跳过）：**
   ...
   ```bash
   # 记录 Bug
   aidevos log bug --title "Bug简述" --severity high --source self-review --task TASK-XX --files "file1.ts,file2.ts"
   # 修复后标记
   aidevos log bug-fix --id BUG-XX --fix "修复方案描述"
   # 记录修改的文件
   aidevos log file --path "修改的文件路径" --change-type modified --lines-added XX --lines-removed XX
   ```
