...
1. **确定当前任务并记录开始时间：**
   ...
   ```bash
   aidevos log task-start --id TASK-XX
   ```
...
4. **状态更新（强制，不可跳过）：**
   ...
   **a) 记录所有文件变更（必须，不能遗漏）：**
   ```bash
   aidevos log file --path "src/views/xxx.vue" --change-type modified --lines-added 50 --lines-removed 10
   aidevos log file --path "src/api/yyy.ts" --change-type created --lines-added 30
   ```
   - **对每个修改过的文件都必须执行一次** `aidevos log file`
