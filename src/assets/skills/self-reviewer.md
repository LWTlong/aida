...
3. **结果输出（强制，不可跳过）：**
   ...
   ```bash
   # 通过时不传 --issues
   aidevos log review --task-id TASK-XX --result pass --scope "审查覆盖的文件/模块"
   # 未通过时，--issues 传逗号分隔的问题描述
   aidevos log review --task-id TASK-XX --result fail --scope "审查覆盖的文件/模块" --issues "问题1描述,问题2描述,问题3描述"
   ```
