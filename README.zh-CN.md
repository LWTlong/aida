...
**AIDA 采集每一个数据点，用看板可视化，再把偏差规律沉淀成规则 —— 让你的 AI 每次运行都写出更符合预期的代码。**
...
| "这个错误我纠正三次了" | AIDA 记录了偏差模式，AI 通过 deviation-recorder skill 创建了规则。AI 每次都读 —— 你再也不用纠正 |
...
> 用 Claude Code 一周了。打开 AIDA 看板：23 个偏差，集中在 `组件使用` 和 `布局` 类别，根因大部分是 `规则缺失`。通过 deviation-recorder skill，AI 识别模式并创建了 6 条项目规则。下一周，这些类别偏差归零。AI 现在懂你的项目规范了。
